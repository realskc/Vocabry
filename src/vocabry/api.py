...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    app = FastAPI(title="Vocabry", version="0.1.0", lifespan=lifespan, docs_url=None, redoc_url=None, openapi_url=None)
    ...
    def list_cards(
        ...
        cards = service.database.list_cards(include_deleted=include_deleted, limit=limit, offset=offset)
...
<h2>Front</h2><iframe sandbox srcdoc="{front}"></iframe><h2>Back</h2><iframe sandbox srcdoc="{back}"></iframe>"""
    ...
    def reconcile(_=Depends(current_client)) -> dict[str, str]:
